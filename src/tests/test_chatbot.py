...
# ----------------------------
#
# These tests are designed to validate ANY Chatbot API implementation.
# Your implementation must provide a fixture named `chatbot_client`.
#
...
def chatbot_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Chatbot OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Chatbot.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(chatbot_client: Any):
    if callable(chatbot_client):
        return chatbot_client
    req = getattr(chatbot_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `chatbot_client` fixture must return either a callable or an object "
...
@pytest.fixture
def chatbot_cases(chatbot_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(chatbot_spec)
    if not cases:
...
def test_chatbot_spec_is_openapi_3(chatbot_spec: dict[str, Any]) -> None:
    assert chatbot_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert chatbot_spec.get("info", {}).get("title") == "Chatbot"
    assert "paths" in chatbot_spec and isinstance(chatbot_spec["paths"], Mapping)


def test_chatbot_operations_have_operation_ids(chatbot_cases: list[OperationCase]) -> None:
    missing = [c for c in chatbot_cases if not c.operation_id]
    assert not missing, "All operations should have an operationId (or derived id)."


def test_chatbot_embedded_json_schemas_validate(chatbot_cases: list[OperationCase]) -> None:
...
def test_chatbot_placeholder(case: Any) -> None:
    assert case is None
...
def test_chatbot_operation_contract(
    ...
    chatbot_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(chatbot_client)

    formatted_path = _format_path(chatbot_case.path, chatbot_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if chatbot_case.response_schema is not None:
        response_payload = _example_from_schema(chatbot_spec, chatbot_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, chatbot_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": chatbot_case.status_code}
    if chatbot_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(chatbot_case.method, url).mock(
    ...
    result = request(
        chatbot_case.method,
        chatbot_case.path,
        path_params=chatbot_case.path_params or None,
        params=chatbot_case.query_params or None,
        json=chatbot_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == chatbot_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {chatbot_case.method} {url}"

    call = route.calls[-1].request

    if chatbot_case.query_params:
        for k, v in chatbot_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if chatbot_case.request_json is not None and chatbot_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == chatbot_case.request_json

    if chatbot_case.response_schema is not None:
        _validate(got, chatbot_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_chatbot_client_uses_httpx_transport(chatbot_client: Any) -> None:
    req = _get_request_callable(chatbot_client)
    assert callable(req)
