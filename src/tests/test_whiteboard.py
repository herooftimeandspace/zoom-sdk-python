...
# ----------------------------
#
# These tests are designed to validate ANY Whiteboard API implementation.
# Your implementation must provide a fixture named `whiteboard_client`.
#
...
def whiteboard_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Whiteboard OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Whiteboard.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(whiteboard_client: Any):
    if callable(whiteboard_client):
        return whiteboard_client
    req = getattr(whiteboard_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `whiteboard_client` fixture must return either a callable or an object "
...
@pytest.fixture
def whiteboard_cases(whiteboard_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(whiteboard_spec)
    if not cases:
...
def test_whiteboard_spec_is_openapi_3(whiteboard_spec: dict[str, Any]) -> None:
    assert whiteboard_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert whiteboard_spec.get("info", {}).get("title") == "Whiteboard"
    assert "paths" in whiteboard_spec and isinstance(whiteboard_spec["paths"], Mapping)


def test_whiteboard_operations_have_operation_ids(
    whiteboard_cases: list[OperationCase],
) -> None:
    missing = [c for c in whiteboard_cases if not c.operation_id]
...
def test_whiteboard_placeholder(case: Any) -> None:
    assert case is None
...
def test_whiteboard_operation_contract(
    ...
    whiteboard_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(whiteboard_client)

    formatted_path = _format_path(whiteboard_case.path, whiteboard_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if whiteboard_case.response_schema is not None:
        response_payload = _example_from_schema(whiteboard_spec, whiteboard_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, whiteboard_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": whiteboard_case.status_code}
    if whiteboard_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(whiteboard_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        whiteboard_case.method,
        whiteboard_case.path,
        path_params=whiteboard_case.path_params or None,
        params=whiteboard_case.query_params or None,
        json=whiteboard_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == whiteboard_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {whiteboard_case.method} {url}"

    call = route.calls[-1].request

    if whiteboard_case.query_params:
        for k, v in whiteboard_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if whiteboard_case.request_json is not None and whiteboard_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == whiteboard_case.request_json

    if whiteboard_case.response_schema is not None:
        _validate(got, whiteboard_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_whiteboard_client_uses_httpx_transport(whiteboard_client: Any) -> None:
    req = _get_request_callable(whiteboard_client)
    assert callable(req)
