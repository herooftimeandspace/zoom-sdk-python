...
# ----------------------------
#
# These tests are designed to validate ANY Zoom Docs API implementation.
# Your implementation must provide a fixture named `zoom_docs_client`.
#
...
def zoom_docs_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Zoom Docs OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Zoom Docs.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(zoom_docs_client: Any):
    if callable(zoom_docs_client):
        return zoom_docs_client
    req = getattr(zoom_docs_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `zoom_docs_client` fixture must return either a callable or an object "
...
@pytest.fixture
def zoom_docs_cases(zoom_docs_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(zoom_docs_spec)
    if not cases:
...
def test_zoom_docs_spec_is_openapi_3(zoom_docs_spec: dict[str, Any]) -> None:
    assert zoom_docs_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert zoom_docs_spec.get("info", {}).get("title") == "Zoom Docs"
    assert "paths" in zoom_docs_spec and isinstance(zoom_docs_spec["paths"], Mapping)


def test_zoom_docs_operations_have_operation_ids(
    zoom_docs_cases: list[OperationCase],
) -> None:
    missing = [c for c in zoom_docs_cases if not c.operation_id]
...
def test_zoom_docs_placeholder(case: Any) -> None:
    assert case is None
...
def test_zoom_docs_operation_contract(
    ...
    zoom_docs_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(zoom_docs_client)

    formatted_path = _format_path(zoom_docs_case.path, zoom_docs_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if zoom_docs_case.response_schema is not None:
        response_payload = _example_from_schema(zoom_docs_spec, zoom_docs_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, zoom_docs_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": zoom_docs_case.status_code}
    if zoom_docs_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(zoom_docs_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        zoom_docs_case.method,
        zoom_docs_case.path,
        path_params=zoom_docs_case.path_params or None,
        params=zoom_docs_case.query_params or None,
        json=zoom_docs_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == zoom_docs_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {zoom_docs_case.method} {url}"

    call = route.calls[-1].request

    if zoom_docs_case.query_params:
        for k, v in zoom_docs_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if zoom_docs_case.request_json is not None and zoom_docs_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == zoom_docs_case.request_json

    if zoom_docs_case.response_schema is not None:
        _validate(got, zoom_docs_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_zoom_docs_client_uses_httpx_transport(zoom_docs_client: Any) -> None:
    req = _get_request_callable(zoom_docs_client)
    assert callable(req)
