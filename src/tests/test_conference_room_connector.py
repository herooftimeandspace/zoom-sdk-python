...
SPEC_PATH = (
    Path(__file__).resolve().parent / "schemas" / "workplace" / "Conference Room Connector.json"
)
...
# ----------------------------
#
# These tests are designed to validate ANY Conference Room Connector API implementation.
# Your implementation must provide a fixture named `conference_room_connector_client`.
#
...
def conference_room_connector_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Conference Room Connector OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Conference Room Connector.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(conference_room_connector_client: Any):
    if callable(conference_room_connector_client):
        return conference_room_connector_client
    req = getattr(conference_room_connector_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `conference_room_connector_client` fixture must return either a callable "
        "or an object with a callable `.request(...)` method."
    )
...
@pytest.fixture
def conference_room_connector_cases(
    conference_room_connector_spec: dict[str, Any],
) -> list[OperationCase]:
    cases = _build_operation_cases(conference_room_connector_spec)
    if not cases:
...
def test_conference_room_connector_spec_is_openapi_3(
    ...
    assert conference_room_connector_spec.get("openapi", "").startswith(
        "3."
    ), "Expected OpenAPI 3.x spec"
    assert conference_room_connector_spec.get("info", {}).get("title") == "Conference Room Connector"
...
def test_conference_room_connector_operations_have_operation_ids(
    conference_room_connector_cases: list[OperationCase],
) -> None:
    missing = [c for c in conference_room_connector_cases if not c.operation_id]
...
def test_conference_room_connector_embedded_json_schemas_validate(
    ...
    for case in conference_room_connector_cases:
...
def test_conference_room_connector_placeholder(case: Any) -> None:
    assert case is None
...
def test_conference_room_connector_operation_contract(
    ...
    conference_room_connector_spec: dict[str, Any],
    conference_room_connector_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(conference_room_connector_client)

    formatted_path = _format_path(
        conference_room_connector_case.path, conference_room_connector_case.path_params
    )
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if conference_room_connector_case.response_schema is not None:
        response_payload = _example_from_schema(
            conference_room_connector_spec, conference_room_connector_case.response_schema
        )
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, conference_room_connector_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": conference_room_connector_case.status_code}
    if conference_room_connector_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(conference_room_connector_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        conference_room_connector_case.method,
        conference_room_connector_case.path,
        path_params=conference_room_connector_case.path_params or None,
        params=conference_room_connector_case.query_params or None,
        json=conference_room_connector_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == conference_room_connector_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {conference_room_connector_case.method} {url}"

    call = route.calls[-1].request

    if conference_room_connector_case.query_params:
        for k, v in conference_room_connector_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if (
        conference_room_connector_case.request_json is not None
        and conference_room_connector_case.method in {"POST", "PUT", "PATCH"}
    ):
        ...
        assert sent == conference_room_connector_case.request_json

    if conference_room_connector_case.response_schema is not None:
        _validate(got, conference_room_connector_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_conference_room_connector_client_uses_httpx_transport(
    conference_room_connector_client: Any,
) -> None:
    req = _get_request_callable(conference_room_connector_client)
    assert callable(req)
