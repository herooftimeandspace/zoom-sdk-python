...
# ----------------------------
#
# These tests are designed to validate ANY Mail API implementation.
# Your implementation must provide a fixture named `mail_client`.
#
...
def mail_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Mail OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Mail.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(mail_client: Any):
    if callable(mail_client):
        return mail_client
    req = getattr(mail_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `mail_client` fixture must return either a callable or an object "
...
@pytest.fixture
def mail_cases(mail_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(mail_spec)
    if not cases:
...
def test_mail_spec_is_openapi_3(mail_spec: dict[str, Any]) -> None:
    assert mail_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert mail_spec.get("info", {}).get("title") == "Mail"
    assert "paths" in mail_spec and isinstance(mail_spec["paths"], Mapping)


def test_mail_operations_have_operation_ids(mail_cases: list[OperationCase]) -> None:
    missing = [c for c in mail_cases if not c.operation_id]
    assert not missing, "All operations should have an operationId (or derived id)."


def test_mail_embedded_json_schemas_validate(mail_cases: list[OperationCase]) -> None:
...
def test_mail_placeholder(case: Any) -> None:
    assert case is None
...
def test_mail_operation_contract(
    ...
    mail_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(mail_client)

    formatted_path = _format_path(mail_case.path, mail_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if mail_case.response_schema is not None:
        response_payload = _example_from_schema(mail_spec, mail_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, mail_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": mail_case.status_code}
    if mail_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(mail_case.method, url).mock(
    ...
    result = request(
        mail_case.method,
        mail_case.path,
        path_params=mail_case.path_params or None,
        params=mail_case.query_params or None,
        json=mail_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == mail_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {mail_case.method} {url}"

    call = route.calls[-1].request

    if mail_case.query_params:
        for k, v in mail_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if mail_case.request_json is not None and mail_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == mail_case.request_json

    if mail_case.response_schema is not None:
        _validate(got, mail_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_mail_client_uses_httpx_transport(mail_client: Any) -> None:
    req = _get_request_callable(mail_client)
    assert callable(req)
