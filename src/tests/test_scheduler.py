...
# ----------------------------
#
# These tests are designed to validate ANY Scheduler API implementation.
# Your implementation must provide a fixture named `scheduler_client`.
#
...
def scheduler_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Scheduler OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Scheduler.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(scheduler_client: Any):
    if callable(scheduler_client):
        return scheduler_client
    req = getattr(scheduler_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `scheduler_client` fixture must return either a callable or an object "
...
@pytest.fixture
def scheduler_cases(scheduler_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(scheduler_spec)
    if not cases:
...
def test_scheduler_spec_is_openapi_3(scheduler_spec: dict[str, Any]) -> None:
    assert scheduler_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert scheduler_spec.get("info", {}).get("title") == "Scheduler"
    assert "paths" in scheduler_spec and isinstance(scheduler_spec["paths"], Mapping)


def test_scheduler_operations_have_operation_ids(
    scheduler_cases: list[OperationCase],
) -> None:
    missing = [c for c in scheduler_cases if not c.operation_id]
...
def test_scheduler_placeholder(case: Any) -> None:
    assert case is None
...
def test_scheduler_operation_contract(
    ...
    scheduler_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(scheduler_client)

    formatted_path = _format_path(scheduler_case.path, scheduler_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if scheduler_case.response_schema is not None:
        response_payload = _example_from_schema(scheduler_spec, scheduler_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, scheduler_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": scheduler_case.status_code}
    if scheduler_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(scheduler_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        scheduler_case.method,
        scheduler_case.path,
        path_params=scheduler_case.path_params or None,
        params=scheduler_case.query_params or None,
        json=scheduler_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == scheduler_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {scheduler_case.method} {url}"

    call = route.calls[-1].request

    if scheduler_case.query_params:
        for k, v in scheduler_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if scheduler_case.request_json is not None and scheduler_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == scheduler_case.request_json

    if scheduler_case.response_schema is not None:
        _validate(got, scheduler_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_scheduler_client_uses_httpx_transport(scheduler_client: Any) -> None:
    req = _get_request_callable(scheduler_client)
    assert callable(req)
