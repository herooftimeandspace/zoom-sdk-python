...
# ----------------------------
#
# These tests are designed to validate ANY Accounts API implementation.
# Your implementation must provide a fixture named `accounts_client`.
#
...
def accounts_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Accounts OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/accounts/Accounts.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(accounts_client: Any):
    if callable(accounts_client):
        return accounts_client
    req = getattr(accounts_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `accounts_client` fixture must return either a callable or an object "
...
@pytest.fixture
def accounts_cases(accounts_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(accounts_spec)
    if not cases:
...
def test_accounts_spec_is_openapi_3(accounts_spec: dict[str, Any]) -> None:
    assert accounts_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert accounts_spec.get("info", {}).get("title") == "Accounts"
    assert "paths" in accounts_spec and isinstance(accounts_spec["paths"], Mapping)


def test_accounts_operations_have_operation_ids(
    accounts_cases: list[OperationCase],
) -> None:
    missing = [c for c in accounts_cases if not c.operation_id]
...
def test_accounts_placeholder(case: Any) -> None:
    assert case is None
...
def test_accounts_operation_contract(
    ...
    accounts_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(accounts_client)

    formatted_path = _format_path(accounts_case.path, accounts_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if accounts_case.response_schema is not None:
        response_payload = _example_from_schema(accounts_spec, accounts_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, accounts_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": accounts_case.status_code}
    if accounts_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(accounts_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        accounts_case.method,
        accounts_case.path,
        path_params=accounts_case.path_params or None,
        params=accounts_case.query_params or None,
        json=accounts_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == accounts_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {accounts_case.method} {url}"

    call = route.calls[-1].request

    if accounts_case.query_params:
        for k, v in accounts_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if accounts_case.request_json is not None and accounts_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == accounts_case.request_json

    if accounts_case.response_schema is not None:
        _validate(got, accounts_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_accounts_client_uses_httpx_transport(accounts_client: Any) -> None:
    req = _get_request_callable(accounts_client)
    assert callable(req)
