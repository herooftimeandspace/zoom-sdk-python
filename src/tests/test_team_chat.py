...
# ----------------------------
#
# These tests are designed to validate ANY Team Chat API implementation.
# Your implementation must provide a fixture named `team_chat_client`.
#
...
def team_chat_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Team Chat OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Team Chat.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(team_chat_client: Any):
    if callable(team_chat_client):
        return team_chat_client
    req = getattr(team_chat_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `team_chat_client` fixture must return either a callable or an object "
...
@pytest.fixture
def team_chat_cases(team_chat_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(team_chat_spec)
    if not cases:
...
def test_team_chat_spec_is_openapi_3(team_chat_spec: dict[str, Any]) -> None:
    assert team_chat_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert team_chat_spec.get("info", {}).get("title") == "Team Chat"
    assert "paths" in team_chat_spec and isinstance(team_chat_spec["paths"], Mapping)


def test_team_chat_operations_have_operation_ids(
    team_chat_cases: list[OperationCase],
) -> None:
    missing = [c for c in team_chat_cases if not c.operation_id]
...
def test_team_chat_placeholder(case: Any) -> None:
    assert case is None
...
def test_team_chat_operation_contract(
    ...
    team_chat_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(team_chat_client)

    formatted_path = _format_path(team_chat_case.path, team_chat_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if team_chat_case.response_schema is not None:
        response_payload = _example_from_schema(team_chat_spec, team_chat_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, team_chat_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": team_chat_case.status_code}
    if team_chat_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(team_chat_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        team_chat_case.method,
        team_chat_case.path,
        path_params=team_chat_case.path_params or None,
        params=team_chat_case.query_params or None,
        json=team_chat_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == team_chat_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {team_chat_case.method} {url}"

    call = route.calls[-1].request

    if team_chat_case.query_params:
        for k, v in team_chat_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if team_chat_case.request_json is not None and team_chat_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == team_chat_case.request_json

    if team_chat_case.response_schema is not None:
        _validate(got, team_chat_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_team_chat_client_uses_httpx_transport(team_chat_client: Any) -> None:
    req = _get_request_callable(team_chat_client)
    assert callable(req)
