...
# ----------------------------
#
# These tests are designed to validate ANY Phone API implementation.
# Your implementation must provide a fixture named `phone_client`.
#
...
def phone_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Phone OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Phone.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(phone_client: Any):
    if callable(phone_client):
        return phone_client
    req = getattr(phone_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `phone_client` fixture must return either a callable or an object "
...
@pytest.fixture
def phone_cases(phone_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(phone_spec)
    if not cases:
...
def test_phone_spec_is_openapi_3(phone_spec: dict[str, Any]) -> None:
    assert phone_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert phone_spec.get("info", {}).get("title") == "Phone"
    assert "paths" in phone_spec and isinstance(phone_spec["paths"], Mapping)


def test_phone_operations_have_operation_ids(phone_cases: list[OperationCase]) -> None:
    missing = [c for c in phone_cases if not c.operation_id]
    assert not missing, "All operations should have an operationId (or derived id)."


def test_phone_embedded_json_schemas_validate(phone_cases: list[OperationCase]) -> None:
...
def test_phone_placeholder(case: Any) -> None:
    assert case is None
...
def test_phone_operation_contract(
    ...
    phone_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(phone_client)

    formatted_path = _format_path(phone_case.path, phone_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if phone_case.response_schema is not None:
        response_payload = _example_from_schema(phone_spec, phone_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, phone_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": phone_case.status_code}
    if phone_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(phone_case.method, url).mock(
    ...
    result = request(
        phone_case.method,
        phone_case.path,
        path_params=phone_case.path_params or None,
        params=phone_case.query_params or None,
        json=phone_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == phone_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {phone_case.method} {url}"

    call = route.calls[-1].request

    if phone_case.query_params:
        for k, v in phone_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if phone_case.request_json is not None and phone_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == phone_case.request_json

    if phone_case.response_schema is not None:
        _validate(got, phone_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_phone_client_uses_httpx_transport(phone_client: Any) -> None:
    req = _get_request_callable(phone_client)
    assert callable(req)
