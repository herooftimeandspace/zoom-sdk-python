...
# ----------------------------
#
# These tests are designed to validate ANY Rooms API implementation.
# Your implementation must provide a fixture named `rooms_client`.
#
...
def rooms_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Rooms OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Rooms.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(rooms_client: Any):
    if callable(rooms_client):
        return rooms_client
    req = getattr(rooms_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `rooms_client` fixture must return either a callable or an object "
...
@pytest.fixture
def rooms_cases(rooms_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(rooms_spec)
    if not cases:
...
def test_rooms_spec_is_openapi_3(rooms_spec: dict[str, Any]) -> None:
    assert rooms_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert rooms_spec.get("info", {}).get("title") == "Rooms"
    assert "paths" in rooms_spec and isinstance(rooms_spec["paths"], Mapping)


def test_rooms_operations_have_operation_ids(rooms_cases: list[OperationCase]) -> None:
    missing = [c for c in rooms_cases if not c.operation_id]
    assert not missing, "All operations should have an operationId (or derived id)."


def test_rooms_embedded_json_schemas_validate(rooms_cases: list[OperationCase]) -> None:
    for case in rooms_cases:
...
def test_rooms_placeholder(case: Any) -> None:
    assert case is None
...
def test_rooms_operation_contract(
    ...
    rooms_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(rooms_client)

    formatted_path = _format_path(rooms_case.path, rooms_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if rooms_case.response_schema is not None:
        response_payload = _example_from_schema(rooms_spec, rooms_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, rooms_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": rooms_case.status_code}
    if rooms_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(rooms_case.method, url).mock(
    ...
    result = request(
        rooms_case.method,
        rooms_case.path,
        path_params=rooms_case.path_params or None,
        params=rooms_case.query_params or None,
        json=rooms_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == rooms_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {rooms_case.method} {url}"

    call = route.calls[-1].request

    if rooms_case.query_params:
        for k, v in rooms_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if rooms_case.request_json is not None and rooms_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == rooms_case.request_json

    if rooms_case.response_schema is not None:
        _validate(got, rooms_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_rooms_client_uses_httpx_transport(rooms_client: Any) -> None:
    req = _get_request_callable(rooms_client)
    assert callable(req)
