"""Contract tests for the Zoom Healthcare API endpoint group.

Schema source:
  src/tests/schemas/build_platform/Healthcare.json
...
These tests validate *client implementations* (not live servers):
  - They load the OpenAPI spec.
  - They assert the client exposes a `.healthcare` service group.
  - They ensure each OpenAPI operation maps to a callable.
  - They ensure the callable makes the correct HTTP request (method + path) using httpx.
  - They validate returned JSON against the OpenAPI response schema (when present).

If your client differs in naming, adjust the `zoom_client` fixture and `_svc(...)`.
"""
...
SCHEMA_PATH = Path("src/tests/schemas/build_platform/Healthcare.json")
...
def _iter_operations(spec: Mapping[str, Any]) -> Iterable[tuple[str, str, str, dict]]:
    """Yield (operation_id, http_method, path, op)."""
...
def _deep_deref(spec: Mapping[str, Any], schema: Any) -> Any:
    """Recursively inline local $ref entries so Draft202012Validator can validate without a resolver."""
...
def _build_required_kwargs(spec: Mapping[str, Any], op: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any] | None]:
    """Return (kwargs, json_body) for required parameters and (required) requestBody."""
    kwargs: dict[str, Any] = {}
...
@pytest.fixture
def zoom_client():
    """Project-level client fixture.

    Contract expectation for this repo:
      - from zoompy import ZoomClient
      - ZoomClient(token=..., base_url=...)
...
def _svc(client: Any) -> Any:
    if hasattr(client, "healthcare"):
        return client.healthcare
    if hasattr(client, "clinical_notes"):
        return client.clinical_notes
    raise AssertionError("Client must expose Healthcare service at .healthcare (or .clinical_notes)")
...
def _get_method(service: Any, operation_id: str):
    ...
    raise AttributeError(
        f"Healthcare service missing method for operationId '{operation_id}'. "
...
def test_healthcare_group_exists(zoom_client: Any) -> None:
    _ = _svc(zoom_client)
...
def test_healthcare_operations_are_callable(zoom_client: Any, case: OperationCase) -> None:
    svc = _svc(zoom_client)
    _get_method(svc, case.operation_id)


@pytest.mark.parametrize("case", _cases(), ids=lambda c: f"{c.method.upper()} {c.path} ({c.operation_id})")
def test_healthcare_operation_makes_expected_http_call_and_validates_response(
    ...
    # Create a schema-valid response payload if there is a JSON schema.
...
def test_healthcare_missing_required_path_param_fails_fast(zoom_client: Any) -> None:
    """Pick one path-param operation and ensure missing required args raise before HTTP."""
    spec = _load_spec()

    # Find any op with a required path param.
    target = None
    for operation_id, method, path, op in _iter_operations(spec):
    ...
    if target is None:
        pytest.skip("No operations with required path params found in Healthcare schema")
    ...
    with respx.mock(assert_all_called=False, assert_all_mocked=False):
        # We don't know the filled-in path value; just ensure no requests go out.
...
def test_healthcare_schema_file_sanity() -> None:
    spec = _load_spec()
